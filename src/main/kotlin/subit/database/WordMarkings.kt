...
class WordMarkings: DaoSqlImpl<WordMarkings.WordMarkingsTable>(WordMarkingsTable), KoinComponent
{
    ...
    suspend fun getWordMarking(wid: WordMarkingId): WordMarkingInfo? = query()
    {
        selectAll().where { id eq wid }.singleOrNull()?.let { deserialize(it) }
    }

    suspend fun getWordMarking(postVersion: PostVersionId, comment: PostId): WordMarkingInfo? = query()
    {
        selectAll()
    ...
    suspend fun batchAddWordMarking(wordMarkings: List<WordMarkingInfo>): List<WordMarkingId> = query()
    {
    ...
    suspend fun getWordMarkings(postVersion: PostVersionId): List<WordMarkingInfo> = query()
    {
