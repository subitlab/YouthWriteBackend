...
class Prohibits: DaoSqlImpl<Prohibits.ProhibitsTable>(ProhibitsTable)
{
    object ProhibitsTable: IdTable<UserId>("prohibits")
    {
        ...
        val operator = reference("operator", Users.UsersTable).index()
    ...
    suspend fun addProhibit(prohibit: Prohibit): Unit = query()
    {
        insert {
    ...
    suspend fun removeProhibit(uid: UserId): Unit = query()
    {
     ...
     * 检查用户是否被禁止, true代表被封禁
     */
    suspend fun isProhibited(uid: UserId): Boolean = query()
    {
